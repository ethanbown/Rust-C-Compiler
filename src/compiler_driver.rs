...
use crate::{assembly::assembly as assembly, code_emission::code_emission as code_emission, lexer::{PathData as PathData, 
                    lexer as lexer}, parser::parser as parser, tacky::tacky as tacky};
...
fn stop_early(path: &Path, flag: CompilerFlags) {
    match flag {
        ...
        CompilerFlags::StopAtTacky => stop_at_tacky(path),
...
pub fn stop_at_tacky(path: &Path) {
    let path_data = create_pathdata(path);
    run_preprocessor(&path_data);
    let file_path_i = path_data.file_parent.clone() + "/" + path_data.file_stem.as_str() + ".i";
    let mut tokens = lexer(&file_path_i, &path_data);
...
pub fn compile_preprocessed_file(pd: &PathData) {
    ...
    let ast = parser(&mut tokens);
    let tacky_ast = tacky(&ast);
    let binary_ast = assembly(&tacky_ast);
